...
export default function MessageInput({
  ...
  const startRecording = async () => {
    try {
      ...
      alert(
        "Microphone access is required for voice messages. Please allow microphone access.",
      );
    }
  };
  ...
  return (
    <div className="sticky bottom-0 z-20">
      {/* Reply preview */}
      {replyTo && (
        <div className="bg-[#1e2a30] border-l-4 border-[#00a884] px-4 py-2 flex items-center justify-between mx-2 mt-1 rounded-t-xl shadow-sm">
          ...
          <button
            ...
            className="text-[#8696a0] hover:text-[#e9edef] ml-3 flex-shrink-0"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
      ...
      {/* Upload progress */}
      {uploading && (
        <div className="bg-[#1e2a30] px-4 py-2 mx-2 flex items-center gap-2 text-[#8696a0] text-sm rounded-xl shadow-sm">
      ...
      {showAttachMenu && (
        <>
          ...
          />
          <div className="absolute bottom-full left-12 mb-3 bg-[#233138] rounded-2xl shadow-2xl py-3 px-2 pop-up z-[30] border border-[#222d34]">
      ...
      {/* Main input bar */}
      <div className="bg-[#0B120D] px-4 py-2 flex items-center gap-3 border-t border-[#222d34] shadow-[0_-1px_0_0_rgba(0,0,0,0.08)]">
        {isRecording ? (
          /* ─── Recording UI ─── */
          ...
          <>
            <div className="flex items-center gap-4 text-[#aebac1]">
              <button
                ...
                className={`p-2 rounded-lg hover:bg-[#2a3942] hover:text-[#e9edef] transition-colors ${showEmojiPicker ? "text-[#00a884]" : ""}`}
              >
                {showEmojiPicker ? (
                  <X className="w-6 h-6" />
                ) : (
              ...
              <button
                ...
                className={`p-2 rounded-lg hover:bg-[#2a3942] hover:text-[#e9edef] transition-colors ${showAttachMenu ? "text-[#00a884]" : ""}`}
              >
                {showAttachMenu ? (
                  <X className="w-6 h-6" />
                ) : (
            ...
            <textarea
              ...
              rows={1}
            className="flex-1 bg-[#ffff] text-[#000] text-sm py-2.5 px-4 rounded-xl outline-none placeholder:text-[#8696a0] resize-none max-h-32 border border-transparent focus:border-[#00a884]/40 transition-colors overflow-hidden"
            onInput={(e) => {
            ...
            <div className="text-[#aebac1] flex-shrink-0">
              {content.trim() ? (
                <button
                  onClick={handleSend}
                  disabled={sending}
                  className="p-2 rounded-lg text-[#00a884] hover:bg-[#2a3942] hover:text-[#008069] transition-colors disabled:opacity-50"
                >
                  <Send className="w-6 h-6" />
                </button>
              ) : (
                <button
                  ...
                  className="p-2 rounded-lg hover:bg-[#2a3942] hover:text-[#e9edef] transition-colors"
                >
...
function AttachOption({
  ...
  return (
    <button
      onClick={onClick}
      className="flex flex-col items-center gap-1.5 p-2 rounded-xl hover:bg-[#111b21] transition-colors"
    >
      <div
        className={`w-10 h-10 ${color} rounded-full flex items-center justify-center text-white`}
      >
        {icon}
      </div>
