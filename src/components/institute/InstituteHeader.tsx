...
export default function InstituteHeader({
  ...
  // For teachers, split tabs: first 6 on row 1, Discussion + Attendance on row 2
  const teacherRow1Ids = ["overview", "quizzes", "assignments", "live-classes", "grades", "announcements", "discussion","resources"];
  const teacherRow2Ids = [ "attendance"];
  const isTeacher = userRole === "teacher";
  const row1Tabs = isTeacher ? tabs.filter(t => teacherRow1Ids.includes(t.id)) : tabs;
  const row2Tabs = isTeacher ? tabs.filter(t => teacherRow2Ids.includes(t.id)) : [];

  const tabBtn = (tab: { id: string; label: string; icon: React.ReactNode }) => (
    <button
      key={tab.id}
