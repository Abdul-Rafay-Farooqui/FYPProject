...
export default function TeachersPage() {
  ...
  const fetchTeachers = async () => {
    try {
      const data = await api.get(`/api/cms/users?school_id=${school!.id}&role=teacher`);
      setTeachers(data);
    } catch (err: any) { setError(err.response?.data?.message || err.message || 'Error fetching teachers'); } finally { setLoading(false); }
  };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await api.post('/api/cms/users', { ...formData, school_role: 'teacher', school_id: school!.id });
      setShowModal(false);
      setFormData({ display_name: '', email: '' });
      setError('');
      fetchTeachers();
    } catch (err: any) { setError(err.response?.data?.message || err.message || 'Error creating teacher'); }
  };

  const handleDelete = async (id: string) => {
    if (!confirm('Delete this teacher?')) return;
    try { await api.delete(`/api/cms/users/${id}`); setError(''); fetchTeachers(); } catch (err: any) { setError(err.response?.data?.message || err.message || 'Error deleting teacher'); }
  };
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-xl font-bold text-[#e9edef]">Teachers</h1>
      ...
      <div className="rounded-xl overflow-hidden" style={{ background: '#111b21', border: '1px solid #222d34' }}>
        <table className="min-w-full">
          ...
          <tbody className="divide-y divide-[#222d34]">
            {teachers.map((t) => (
              <tr key={t.id}>
                <td className="px-6 py-4 text-sm text-[#e9edef]">{t.display_name}</td>
                <td className="px-6 py-4 text-sm text-[#8696a0]">{t.email}</td>
                <td className="px-6 py-4"><button onClick={() => handleDelete(t.id)} className="text-red-400 hover:text-red-300"><Trash2 className="w-4 h-4" /></button></td>
              </tr>
            ))}
            {teachers.length === 0 && <tr><td colSpan={3} className="px-6 py-8 text-center text-[#8696a0]">No teachers yet. Click "Add Teacher" to get started.</td></tr>}
          </tbody>
        </table>
      </div>
