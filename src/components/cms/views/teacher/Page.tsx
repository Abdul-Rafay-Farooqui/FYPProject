'use client';

import { useEffect, useState } from 'react';
import { api } from '@/lib/cms/api';
import { useAuth } from '@/contexts/cms/AuthContext';
import { Users, BookOpen, FileText } from 'lucide-react';

export default function TeacherDashboard() {
  const { user } = useAuth();
  const [stats, setStats] = useState({ assignedClasses: 0, pendingHomework: 0, publishedResults: 0 });
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');

  useEffect(() => { if (user) fetchStats(); }, [user]);

  const fetchStats = async () => {
    try {
      let assignedClasses = 0;
      let pendingHomework = 0;
      let publishedResults = 0;

      try {
        const taRes = await api.get(`/api/teacher-assignments?teacher_id=${user?.id}`);
        assignedClasses = (taRes || []).length;
      } catch { /* ignore */ }

      try {
        const resultsRes = await api.get(`/api/results/count?teacher_id=${user?.id}`);
        publishedResults = resultsRes.count || 0;
      } catch { /* ignore */ }

      setStats({ assignedClasses, pendingHomework, publishedResults });
    } catch (err: any) {
      setError(err.message);
    } finally {
      setLoading(false);
    }
  };

  if (loading) return <div className="flex items-center justify-center h-64"><div className="w-8 h-8 border-2 border-[#00a884]/30 border-t-[#00a884] rounded-full animate-spin" /></div>;

  const statCards = [
    { title: 'Assigned Classes', value: stats.assignedClasses, icon: Users, gradient: 'from-blue-500 to-blue-600' },
    { title: 'Pending Reviews', value: stats.pendingHomework, icon: BookOpen, gradient: 'from-amber-500 to-amber-600' },
    { title: 'Published Results', value: stats.publishedResults, icon: FileText, gradient: 'from-emerald-500 to-emerald-600' },
  ];

  return (
    <div>
      <h1 className="text-xl font-bold text-[#e9edef] mb-6">Welcome, {user?.display_name}</h1>
      {error && <div className="mb-4 p-3 rounded-lg text-sm" style={{ background: 'rgba(239,68,68,0.15)', color: '#f87171' }}>{error}</div>}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {statCards.map((card) => {
          const Icon = card.icon;
          return (
            <div key={card.title} className="rounded-xl p-5" style={{ background: "#111b21", border: "1px solid #222d34" }} style={{ background: '#111b21', border: '1px solid #222d34' }}>
              <div className="flex items-center justify-between mb-3">
                <div className={`bg-gradient-to-br ${card.gradient} p-2.5 rounded-lg`}>
                  <Icon className="w-5 h-5 text-white" />
                </div>
              </div>
              <p className="text-[#8696a0] text-xs mb-0.5">{card.title}</p>
              <p className="text-2xl font-bold text-[#e9edef]">{card.value}</p>
            </div>
          );
        })}
      </div>
    </div>
  );
}
